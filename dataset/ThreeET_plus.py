...
class ThreeETplus_Eyetracking(Dataset):
    ...
    def __init__(
        ...
        base_data_dir = "/content/event_data"  # Update this to the correct base directory
        ...
        if split == "train" or split == "val":
            self.data = [os.path.join(base_data_dir, "train", f, f + ".h5") for f in filenames]
            self.targets = [os.path.join(base_data_dir, "train", f, "label.txt") for f in filenames]
        elif split == "test":
            self.data = [os.path.join(base_data_dir, "test", f, f + ".h5") for f in filenames]
            # for the test set, load the placeholder labels with all zeros
            self.targets = [os.path.join(base_data_dir, "test", f, "label_zeros.txt") for f in filenames]
